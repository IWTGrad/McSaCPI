...
def create_data():
    data_path = '.'
    drugdata_file = data_path + '/smiles.txt'
    prodataw_file = data_path + '/protein.txt'
    data_file = data_path + '/cele_data.csv'
    ...
    pro_embedding = torch.load('./protein_embedding/cele_ProtTransAlbertBFDEmbedder_Pro_Embedding.pt')
    drug_embedding = torch.load('./drug_embedding/cele_MolecularTransformerEmbeddings.pt')
    train_drug, train_protein, train_interaction = np.asarray(train_data.T[0]), \
                                                   np.asarray(train_data.T[1]),np.asarray(train_data.T[2])
    train_datasets = DTADataset(root='data', dataset='cele_ProtTransAlbertBFDEmbedder_MolecularTransformer_train', drug=train_drug,
                                interaction=train_interaction, smile_graph=smile_graph,
                                target_protein=train_protein, pro_embedding=pro_embedding,
    ...
    test_datasets = DTADataset(root='data', dataset='cele_ProtTransAlbertBFDEmbedder_MolecularTransformer_test', drug=test_drug,
                               interaction=test_interaction, smile_graph=smile_graph,
                               target_protein=test_protein, pro_embedding=pro_embedding,
                               drug_embedding=drug_embedding)
