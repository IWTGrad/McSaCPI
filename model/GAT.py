...
from torch_geometric.nn import GENConv,GCNConv,GATConv, global_max_pool as gmp
import numpy as np
...
class GATNet(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2):
        ...
        self.drug_gat1= GATConv(in_channels=num_features_xd,out_channels=num_features_xd,heads=10)
        self.drug_gat2 = GATConv(in_channels=num_features_xd*10, out_channels=num_features_xd, heads=4)
        ...
        self.muti_attention = MultiHeadAttention(input_dim=128 * 3, n_heads=1)
        # combined layers
    ...
    def forward(self, data):
        ...
        x = self.drug_gat1(drug_graph_feature, drug_edge_index)
        x = self.relu(x)

        x = self.drug_gat2(x, drug_edge_index)
        x = self.relu(x)
        ...
        drug_embedding = self.relu(drug_embedding)
        # concat
        xc = torch.cat((x,drug_embedding, pro_embedding), 1)
