...
from torch_geometric.nn import GENConv,GCNConv,GATConv,GatedGraphConv,global_max_pool as gmp
import numpy as np
...
class GCNNet(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.2):
        ...
        self.drug_conv1 = GatedGraphConv(num_features_xd, num_features_xd)
        self.drug_conv2 = GatedGraphConv(num_features_xd, num_features_xd*2)
        self.drug_conv3 = GatedGraphConv(num_features_xd*2, num_features_xd * 4)
        ...
        self.CTD_fc1=nn.Linear(147,1024)
        self.CTD_fc2=nn.Linear(1024,128)

        self.drug_embedding_fc1 = nn.Linear(512, 1024)
        self.drug_embedding_fc2 = nn.Linear(1024, 128)

        self.muti_attention=MultiHeadAttention(input_dim=128*4,n_heads=1)
        # combined layers
        self.fc1 = nn.Linear(8*output_dim, 1024)
    ...
    def forward(self, data):
        ...
        pro_embedding, protein_CTD,correct_interaction = data.edge_index, data.x, data.smiles, data.proseq, data.pro_CTD,data.y
        ...
        #x = self.drug_deepconv(drug_graph_feature, drug_edge_index)
        #x = self.relu(x)
        ...
        protein_CTD=self.CTD_fc1(protein_CTD)
        protein_CTD=self.CTD_fc2(protein_CTD)
        # flatten
        ...
        drug_embedding = self.relu(drug_embedding)
        # concat
        xc = torch.cat((x,drug_embedding ,pro_embedding,protein_CTD), 1)
